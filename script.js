...
const NIGHT_WOLF_PHASE_ID = "__night-wolf-phase__";

const CAMP_THREAD_META = {
  village: {
    title: "Fil prive",
    description: "Messages visibles uniquement pendant le passage autorise.",
    emptyText: "Aucun message prive pour l'instant.",
    placeholder: "Ecris un message discret...",
...
function buildWolfNightTurnOrder(game) {
  return shuffleArray(
    getLivingPlayers(game)
      .filter((player) => isWolfRole(player.role))
      .map((player) => player.id)
  );
}

function buildNightTurnOrder(game, wolfTurnOrder = buildWolfNightTurnOrder(game)) {
  const groups = [[], [], [], []];
  ...
  if (wolfTurnOrder.length) {
    groups[1].push(NIGHT_WOLF_PHASE_ID);
  }
...
function isCurrentNightTurnWolfPhase(game) {
  return Boolean(
    game.night &&
      game.night.order[game.night.turnIndex] === NIGHT_WOLF_PHASE_ID
  );
}
...
function getCurrentNightPlayer(game) {
  ...
  if (isCurrentNightTurnWolfPhase(game)) {
    const currentWolfId = game.night.wolfTurnOrder[game.night.wolfTurnIndex];
    return getPlayer(game, currentWolfId);
  }
...
function getNightStageLabel(game) {
  ...
  for (let index = 0; index < game.night.turnIndex; index += 1) {
    completedTurns +=
      game.night.order[index] === NIGHT_WOLF_PHASE_ID
        ? game.night.wolfTurnOrder.length
        : 1;
  }

  const total =
    game.night.order.length -
    (game.night.order.includes(NIGHT_WOLF_PHASE_ID) ? 1 : 0) +
    game.night.wolfTurnOrder.length;
  const current = isCurrentNightTurnWolfPhase(game)
    ? completedTurns + game.night.wolfTurnIndex + 1
    : completedTurns + 1;
...
function beginNight() {
  ...
  const wolfTurnOrder = buildWolfNightTurnOrder(game);
  game.nightNumber += 1;
  game.summary = null;
  game.postSummary = null;

  // Chaque joueur vivant passe une seule fois par nuit.
  // Les loups ont une sous-phase dediee pour garantir un vote par loup vivant.
  game.night = {
    order: buildNightTurnOrder(game, wolfTurnOrder),
    turnIndex: 0,
    wolfTurnOrder,
    wolfTurnIndex: 0,
...
function moveToNextNightTurn() {
  ...
  if (
    isCurrentNightTurnWolfPhase(game) &&
    game.night.wolfTurnIndex < game.night.wolfTurnOrder.length - 1
  ) {
    game.night.wolfTurnIndex += 1;
    game.scene = "night-pass";
    clearUiError();
    return;
  }
  ...
  if (isCurrentNightTurnWolfPhase(game)) {
    // La victime de la meute est figee une seule fois, apres le dernier vote loup.
...
function renderWolvesContent(player) {
    ...
    return renderNightActionPanel(
      ...
      `
        <form class="stack" data-form="night-action">
          ...
          <div class="info-box">
            Choisis deux joueurs differents a lier en secret.
          </div>
          <div class="target-list">
  ...
  if (isPlayerSabotaged(state.game, player.id)) {
    return renderNightCampAccess(player, {
      noticeText: "Ton action pour cette etape est bloquee. Tu peux toujours utiliser le fil prive.",
      noticeClass: "error-box",
    });
  }

  // Un loup vivant ne peut viser ni lui-meme ni un autre loup vivant.
  ...
  if (!targets.length) {
    return renderNightCampAccess(player, {
      noticeText: "Aucune cible valide n'est disponible a cette etape.",
    });
  }
  ...
  return renderNightActionPanel(
    ...
    `
      <form class="stack" data-form="night-action">
        ...
        <div class="info-box">
          Choisis une cible en secret. Tous les choix de cette etape seront comptes en fin de nuit.
        </div>
        <div class="target-list">
          ${renderRadioTargetList(targets, "targetId")}
        </div>
          ...
          canUseExtraKill
            ? `
              <div class="inline-grid">
                <div class="field-label">Action supplementaire</div>
                <label class="target-option">
                  <input type="radio" name="extraTargetId" value="" checked>
                  <span>
                    <strong>Ne rien ajouter</strong>
                    <em>Conserver cette action pour plus tard</em>
                  </span>
                </label>
...
function submitNightAction(formData) {
  ...
  if (actionType === "wolf-vote") {
    ...
    if (
      ...
      setUiError("Choisis une cible supplementaire valide.");
      render();
      return;
    }

    const hasExtraWolfAction = canBigBadWolfUseExtraKill(game, player) && extraTargetId;
    ...
    game.night.privateMessage = {
      text: "Ton choix est enregistre.",
      detail: hasExtraWolfAction
        ? "Cette action et l'action supplementaire seront resolues en fin de nuit."
        : "Le resultat final de cette etape sera resolu en fin de nuit.",
    };
  }
